...
def test_extract_face():
    ...
    # Call the function
    result = extract_face(img_path, enforce_detection, typical_dataset_im_size, detector_backend)

    # Assertion
    assert isinstance(result, list)  # Ensure the return type is a list
    assert all(isinstance(face, dict) for face in result)  # Ensure each element is a dictionary
    assert all("face" in face for face in result)  # Ensure each dictionary contains "face" key
    assert all(isinstance(face["face"], np.ndarray) for face in result)  # Ensure "face" value is a numpy array
